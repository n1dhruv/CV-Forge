...
export function AssistantDock(props: AssistantDockProps) {
  const [lastProposal, setLastProposal] = useState<(AssistantProposal & { baseSource: string }) | null>(null)
  const diffProposal = props.proposal ?? lastProposal

  useEffect(() => {
    if (props.proposal) setLastProposal(props.proposal)
  }, [props.proposal])
  ...
  return (
    <section className="no-print shrink-0 border-t bg-canvas" aria-labelledby="assistant-title">
      ...
      <div id="assistant-panel" className="max-h-[48vh] overflow-auto border-t" hidden aria-hidden="true">
          ...
          {diffProposal ? (
            <div hidden={!props.proposal} aria-hidden={!props.proposal}>
              <div className="h-64 min-h-52" role="region" aria-label="Proposed source comparison">
                <DiffEditor original={diffProposal.baseSource} modified={diffProposal.tex_source} originalModelPath="inmemory://resume-assistant/original.tex" modifiedModelPath="inmemory://resume-assistant/proposed.tex" language="latex" theme={props.theme === 'dark' ? 'vs-dark' : 'vs'} options={{ automaticLayout: true, fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, lineHeight: 20, minimap: { enabled: false }, readOnly: true, renderSideBySide: true, scrollBeyondLastLine: false, wordWrap: 'on' }} />
              </div>
            </div>
          ) : null}
        </div>
